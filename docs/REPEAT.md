...
`docs/ARCH300.md` left 297 architectures with 293 forwarding and four stopped.
...
`docs/BIND5.md` §4.1 measured the wall correctly and diagnosed it in one line:
...
**The kernels really were all there.** `docs/BINDINGS.md` was once told "`mish`
...
`docs/LAST7.md` §5.2 sized this precisely and deliberately did not land it. Its
...
`docs/LAST7.md` §5.2's correction to `docs/BIND4.md` was right and is why this
...
upstream. It is the same blindness `docs/LAST7.md` §1.1 avoided by never
...
tensor `repeats` before any dispatch happened. That is `docs/LAST7.md` §5.2's
...
`docs/VOICE3.md` found `var`/`std` nearly invisible to it for exactly that
...
`docs/CAPTURE.md` is the contract: a recorded node whose output shape is not
...
This is upstream's `SymInt` rule — `docs/BIND5.md` §7's rule — applied **per
...
three composites (`zeros`, `ones`, `view`) by `docs/BIND4.md` §3 and corrected
by `docs/BIND5.md` §1. It is now in `_TypeChecker`'s int-list predicate and in
...
### 4.2 A defect, and it is `docs/BIND5.md` §7.2 re-firing one position over
...
`docs/ARGFORM.md` §3 and `docs/BIND5.md` both concluded this is **not our gap**:
...
since `docs/BIND5.md` and it belongs in the count: the four remaining
...
1. `test_last7.py::test_repeat_interleave_with_a_tensor_repeats_is_still_refused_by_name`
   ...
   name, because those are the two halves `docs/LAST7.md` §5.2 said would go red
   and the two a value test cannot see. The `docs/LAST7.md` §7 DOCWATCH marker
   moved with it.
