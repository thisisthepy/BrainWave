...
(`docs/distributed/DISTRIBUTED.md` §4.2, §4.3). But "the transport refuses" is only an
...
`docs/design/DESIGN.md` §10 says `adapt/` must not split into directories by
...
`docs/design/DESIGN.md` §3 withdrew one invented set of lifetime names
...
`docs/design/DESIGN.md` §9 item 5 records the problem this is supposed to solve:
...
(`docs/training/BACKWARD.md` §4.2: nothing can be written *out* of this shim).
...
`docs/training/BACKWARD.md` §4.2 measured 8.780e-05 for a cross-entropy gradient over all
...
`docs/numerics/SEQLEN.md` §1.3's prefill logits sha256 over real SmolLM2-135M, measured
...
bytes out of this shim."* Both halves were wrong, and `docs/training/BACKWARD.md` §14
measures each.
...
* **There is another way out: `tolist()`**, which is what `docs/numerics/SEQLEN.md`'s
  ...
  a ten-step Tent delta (§14.4 of `docs/training/BACKWARD.md`).
...
makes (`docs/graph/CAPTURE.md` §4). The class docstring says so rather than hiding it
behind a fallback nobody asked for. `docs/training/BACKWARD.md` §8's reasoning stands: a
...
`docs/training/BACKWARD.md` §7's pattern and `docs/training/LOSS.md` §5.2's, met once more.
...
`docs/training/BACKWARD.md` §18 is the two rules; this is the measurement they were for.
...
`docs/training/BACKWARD.md` §19's T1–T4 are the four faults.
...
single-assignment story, `docs/graph/CAPTURE.md` §9) or `step()` reseeds the global generator around its
