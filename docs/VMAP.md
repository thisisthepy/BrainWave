...
largest single entry on `docs/ARCH200.md`'s blocked list — four architectures
...
| Did it need a `Repr` arm? | **No, and that is the difference from `docs/COMPLEX2.md`.** A batched value here is a plain dense tensor with extra size-1 dimensions. No arm, no tag, no kernel, no `aten.*` name. | §3 |
...
### 3.1 Not a `Repr` arm, which is the difference from `docs/COMPLEX2.md`
...
That is precisely the class of error `docs/COMPLEX.md` §7 was worried about, and
...
That nullification result is the document. It is the failure `docs/COMPLEX.md`
...
therefore a `Repr` arm after all, with the `docs/COMPLEX2.md` audit that
implies.

**Nothing in `docs/ARCH200.md` demands it.** The demand is two closures over
four index scalars. Building the general thing to serve those would be the
mistake `docs/INT8.md` names: pricing a fork of the world to close a gap that a
fence closes.
...
It is worth being exact about what this means for `docs/ARCH200.md`'s
