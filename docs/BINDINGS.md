...
`docs/REACH.md` is the argument and `tools/golden/reach.py` is the check.
...
table row on. `docs/COMPLEX.md` §6 classified it correctly as a **binding** gap:
...
* `test_tail1.py::test_two_kernels_still_have_no_python_spelling_and_it_is_one_line_each`
  ...
  assertion is unchanged — `docs/BACKWARD9.md` §1's hand-spelled criterion
  stands.
...
literal now. This is the trap `docs/TAIL1.md` §4 already names in another form.
...
`docs/VOICE.md` §4.4 records that `aten.mish.default` was implemented, matched
...
`docs/VOICE.md`'s git history can land the binding in the same change, because
