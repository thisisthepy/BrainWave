...
- `docs/SCALAR.md` established that upstream's scalar *precision* rules are per
...
`docs/BACKWARD4.md` §8 found `mul` and recorded it as one operator's problem;
...
`docs/SCALAR.md` §2.2 recorded that `x *= 0.3` reaches `aten.mul_.Tensor`
...
`add.Scalar` and `add.Tensor` promote by different rules (docs/PROMOTE.md), which
...
3. **It would strand the `.Scalar` kernels.** `docs/SCALAR.md` §2's fix — build
...
`docs/BACKWARD4.md` §8 declined to map `mul.Scalar` to `MulBackward0`, on the
...
`_GradFnNode` boundary docs/BACKWARD4.md §1.3 drew, on the other side.

---

## 5. `Tensor.shape` now answers with `torch.Size` — closed

`docs/DEMAND.md` recorded this unranked because no model asked for it:
...
'numel'`, which is the exact symptom docs/DEMAND.md named. A new public path that
...
docs/TORCH_C.md". It was not; TORCH_C.md has never mentioned it. The real record
was in docs/DEMAND.md, three documents away. Noted because a stale cross-
