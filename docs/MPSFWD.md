...
`docs/SEQLEN.md` §7.4 가 이미 반대로 측정해 두었고(`softmax_body` 는 프로파일에 0 회,
...
`docs/MPS.md` §1.1 이 세운 명제("되읽는 것은 candle 이 아니라 이 크레이트다")가 이 경로에서
...
`n-1` 개의 `narrow`/`add` 쌍이고, `docs/VOICE.md` 의 `cumprod` 와 같은 모양입니다. 순서가
...
(`docs/SEQLEN.md` §7, §8.3, §8.12), Metal 위에서 그 이득은 존재하지 않으므로 폴백은 잃는 것이
...
* **`f64` 가 필요한 모든 것.** 제곱 아닌 `pow`, 부동소수 `cumsum`, `_softmax` 의 `read_flat`
  경로. `docs/MPS.md` §7 이 남긴 그대로이고, candle 쪽 문제입니다.
...
그것이 `docs/MPS.md` 가 파생 검사를 만든 이유이자, 이 라운드가 그 검사에 이름 네 개짜리 조항을
하나 더 붙인 이유입니다.
