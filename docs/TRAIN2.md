...
`docs/RELEASE_0_0_13a0.md` §5 states two of this project's own gaps in one sentence:
...
`docs/BACKWARD8.md` §1 had already met this from the side: its `Conv2d + BatchNorm2d` model hit "a
...
`2d_even_kernel` is there because `docs/RNN.md` measured that `lasr`'s configuration takes an
...
of claim `docs/AUDIT.md` found going stale — and it had gone stale in the *conservative* direction,
...
`docs/BACKWARD.md` §6 is why that is a separate problem and not a bigger version of this one: on a
...
implementation of the window geometry, which is the shape `docs/AUDIT.md` keeps finding go stale, so
...
**N2 is the round's sharpest result.** The finite-difference oracle is the one `docs/BACKWARD.md`
...
visible only to upstream. That is `docs/BACKWARD8.md`'s finding with the sides swapped: there the FD
...
**Accumulation.** `docs/BACKWARD9.md`'s finding is repeated rather than assumed:
...
`docs/BACKWARD7.md` §9.1's three SmolLM2-135M logits digests, reproduced rather than copied: the
script was written from `docs/BACKWARD6.md` §7's recipe — sha256 over
...
All three equal `docs/BACKWARD7.md` §9.1's, `docs/BACKWARD6.md` §7's, `docs/BACKWARD5.md` §6.5.1's
and `docs/BACKWARD4.md` §6.2's. A derivative rule adds no forward op, and the golden harness holds
...
| **documentation corrected** | `docs/RELEASE_0_0_13a0.md` §5's transformer clause is now measurably false (§4) — **not edited by this round**, because that file is not this round's territory; it is named here so the next round has the measurement |
| **deleted** | nothing |
