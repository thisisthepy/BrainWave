...
thing it replaces**, in exactly the shape `docs/ARGFORM.md` forbids, and nothing could
...
| 1b. float / bool / multi-element Tensor in a size list | upstream **refuses**; this build now refuses too | — (the fix is a *narrowing* of docs/BIND4.md §3) |
| 2. `torch.zeros((tuple))` for `fastspeech2_conformer` | already landed by docs/BIND4.md §3; **nothing left to pass** | stops one wall later at `repeat_interleave`, `aten.rs` (§4.2) |
...
| 5. where the `export` wall moves | **`torch.empty_strided`**, `docs/EXPORT.md` §3.1 — item **three** (§3) |
| 6. `docs/BIND3.md` §7's objection to the §8 hand-off | **re-measured and gone** (§3.2) | |
...
`docs/STRIDED.md` §6 called this "an argument form, not a missing op", and it is — but
...
So it is the **same** form `docs/BIND4.md` §3 landed for `torch.zeros`, arriving at a
...
torch.ones / torch.empty / Tensor.view    -> all accept it   (docs/BIND4.md §3 re-confirmed)
```

**It is installed for `new_zeros` alone**, plus the `zeros` that was already there. That
is `docs/ARGFORM.md` §2's scoping and `docs/BIND4.md` §3's own precedent: upstream's rule
...
### 1.3 The defect this round found — `docs/BIND4.md` §3 was too permissive
...
`docs/BIND4.md` §3's own comment said multi-element Tensors were "left to fail on their
own `__int__`", and float and bool were not considered at all. Nothing failed, because
nothing asked. This is the shape `docs/ARGFORM.md` names: **a build that accepts what the
...
`docs/BIND4.md` §3.1 is the live trap in this file: `DeviceContext.__torch_function__`
...
## 2. `torch.fx.Graph()` builds — `docs/EXPORT.md` §6 item 2

Taken **only because item 1 landed first.** `docs/EXPORT.md` §6 forbids filling this in
...
no operators in it. `docs/DISPATCH3.md` landed the entrance in `aten.rs`; modes see
...
**`docs/EXPORT.md` §4.1's census undercounted by three, and the three matter.** It listed
...
`docs/EXPORT.md` §2, which are still item **4** and still staged):
...
**That is `docs/EXPORT.md` §3.1 exactly, item three, at the line §3.1 names.** It is not
...
`docs/EXPORT.md` §6 gives for doing it on its own terms ("`torch.fx` is more than
export's substrate").

### 3.2 `docs/BIND3.md` §7's objection, re-measured — it is gone
...
condition `docs/BIND3.md` §7 held the hand-off on is not satisfiable any more, so
`docs/EXPORT.md` §8's patch is unblocked. It was **not** taken here: it is item 4, it is
...
The `.Scalar`/`.Tensor` disagreement in the first row is `docs/DISPATCH3.md` §5's, above
...
The `torch.zeros((tuple))` gap named for this architecture in `docs/TAIL4.md` §8.2 was
**already closed** by `docs/BIND4.md` §3 and is closed on this tree; re-measured before
anything was written, rather than assumed open because a work item said so. Its wall is
`docs/BIND4.md` §3.2's, unchanged:
...
`docs/ARGFORM.md` §3 measured that this is upstream's own
...
| **defect fixed** | `docs/BIND4.md` §3's coercion accepted float, whole-float and bool Tensors that upstream refuses (§1.3); the generated fast path reproduced only one of `resolve`'s two coercions, so positional and keyword spellings could disagree (§7.2); the `repeat_interleave` refusal claimed a missing kernel that exists (§4.2) |
| **tests added** | 27, in `rust/torch_c/pytests/test_bind5.py`. No test was modified, inverted or deleted anywhere |
| **documentation corrected** | `docs/EXPORT.md` §4.1's `_NodeBase` census listed `_erased`/`_next`/`_prev` as "present and real"; all three were raising getters (§2.1). `docs/BIND3.md` §7's verdict on the §8 hand-off is superseded by measurement (§3.2) |
