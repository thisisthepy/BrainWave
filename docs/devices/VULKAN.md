...
`docs/devices/DEVICE.md` §10 은 게스트 확장 문자열 `ANDROID_EMU_vulkan_shader_float16_int8` 을 근거로
...
> **정정 (VULKAN2.md §4).** 이 "macOS 에는 로더가 없다" 는 전제는 더 이상 맞지 않습니다.
> 디스크에 이미 있는 안드로이드 에뮬레이터가 Vulkan 로더와 ICD 4개를 함께 배포하고 있고,
> 환경변수 둘만 주면 **같은 프로브 바이너리가 수정 없이** 이 맥의 실제 GPU(`Apple M1`,
> `INTEGRATED_GPU`)에서 통과합니다. 위 실패는 "로더를 못 찾았다" 이지 "이 기계에 없다" 가
> 아니었습니다. 다만 그 파일들은 안드로이드 SDK 내부물이므로 **개발 기계 경로일 뿐 배포 경로가
> 아닙니다.**
...
3. **`docs/design/DESIGN.md` §5 의 "소유하는 코드가 작다" 와 정합합니다.** `ash` 는 런타임이 아니라
...
- `git status --short` — 변경은 `rust/vk_probe/` 와 `docs/devices/VULKAN.md` 뿐
...
`docs/perf/PERF.md` §7.3 의 Apple 측정(디코딩 모양 행렬×벡터에서 n=4096 까지 GPU 우세)은 실기 측정의
**가설**이지 답이 아닙니다.
