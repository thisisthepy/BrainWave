...
**이 표는 첫 회차의 것입니다.** 그 뒤로 `empty_like`(docs/models/CKPT2.md §3 이 멈춰 있던 자리),
...
torch/_meta_registrations.py), not a fallthrough. See docs/devices/META.md §7.
```
...
- **`docs/perf/PERF.md` 의 2 층 블록을 다시 재지 않았습니다.** 마이크로벤치가 모델 수준으로 어떻게
...
- **혼합 장치 `copy_` 에서 `meta ← cpu` 를 무동작으로 두었을 때, 수신자의 *뷰*가 무엇을 보는지
  모릅니다.** `docs/kernels/OPS4.md` §8 의 미해결과 만나는 자리인데(`copy_` 는 `replace_with` 입니다),
