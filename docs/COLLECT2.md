...
`docs/RELEASE_0_0_13a0.md` §5 said:
...
| secure aggregation, differential privacy | refuses by name | refuses by name (unchanged, `docs/FEDERATED4.md` §7) |
...
`docs/FEDERATED4.md` §2's ordering contract observable — see §5.
...
out per collective (`docs/FEDERATED4.md` §1). Everything here is expressed as a
...
`docs/AGREE.md` §2's method: a threshold picked by eye is not a result, so each
...
- **Secure aggregation, differential privacy, `new_group`, a cohort of one.**
  Unchanged; `docs/FEDERATED4.md` §7 is still the reason for each.
