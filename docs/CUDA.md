...
`docs/VULKAN2.md` §5.1 is the sizing round that found the asymmetry, and it
...
So the two devices are not the same size of work, and `docs/VULKAN3.md` §1's
...
`docs/MPS.md` had already established why: an accelerator tensor that is an
...
* It is a **third entry** rather than a clause on either existing one, for the
  reason `docs/VULKAN3.md` §1.1 records: the Accelerate entry is gated on
...
`docs/ABI3.md` §5 argues for and what a user's Python 3.14 already demonstrated
...
(docs/CUDA.md §3). The underlying error was: the candle crate has not been
...
(`docs/MPS.md` §3.1). Two accelerators cannot legitimately disagree about it,
...
`docs/VOICE3.md` §6 records that it follows helper calls **one level, by name**,
...
`docs/MPS.md` §2 pulled the `mps` guard and measured what came back: fourteen
...
a Metal property as a general one — the exact mistake `docs/MPSATTN.md` §1
records `docs/MPSFWD.md` making about `_softmax`.

### 5.3 The counters, and why they are not the kind of evidence that can be moved out of the way

`docs/MPSATTN.md` §3.1 records, against its own round, that moving a `read_flat`
...
not a measurement — that is the argument. `docs/MPSATTN.md` §5 makes exactly this
...
`docs/MPS.md` §1.2, where `tril` was accused of computing on the CPU on the
...
docs/MPS.md §6 records what happened the one time that was written `eq`.
...
temptation this round was most exposed to is the one `docs/MPS.md` §1.2 names:
