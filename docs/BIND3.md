...
`docs/VOICE3.md` landed three kernels — `im2col`, `col2im`, `upsample_nearest1d` —
...
The bar was set by `docs/BINDINGS.md`, which was told "`mish` just needs a binding"
...
written, because `docs/BINDINGS.md`'s `mish` is what happens when it is not.
...
among other sources. `docs/BIND2.md` had to write literal dict lookups rather than an
...
branch is not exercised by that forward at all. `docs/VOICE3.md` §1.3 and
`docs/COMPLEX2.md` §8 both flagged this ambiguity and declined to claim it; the
...
(`torch._C._complex_ops()`, `docs/COMPLEX.md` §2). That is the next wall for this tower
...
`docs/FFT.md` §5.3 called this "now a binding rather than arithmetic", and
`docs/ARCH200.md` lists `torch._C._fft.fft_fftn` as `fnet`'s wall. It is *nearly* right:
...
1. **There is no real → complex conversion.** `_to_copy` to a complex dtype is the first
   ...
   (`docs/COMPLEX.md` §2). `view_as_complex` produces something the C2C kernel then
...
`docs/COMPLEX2.md` taught twelve other ops — and `aten.rs` was not this round's file.
...
`docs/EXPORT.md` §6 orders the work: **dispatcher mode entrance first, `_NodeBase`
...
fake-tensor abstraction is silently off. That is `docs/COMPILE.md` §5's silent eager
...
no matching benefit, and `docs/EXPORT.md` §8 is written as one patch for a reason.
...
Split as `docs/ARCH100.md` §5.3 asks, because "landed" is not one thing:
