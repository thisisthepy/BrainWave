...
`docs/ARCH200.md` left 7 of 297 architectures blocked and four came here as
...
anyway**: `docs/ARGFORM.md` §1 had already measured that `padding=[0, 5]` is not
...
`docs/BIND3.md` and is a different kernel in this same file.
...
indexes**, and `docs/STRIDED.md` §1 already established that no composition of
...
while it means anything; `docs/STRIDED.md` §2 and §3 are the argument and none of
...
`as_strided` refuses a non-contiguous receiver by name (`docs/STRIDED.md` §4.1)
...
## 4. The convolution padding, which `docs/ARCH100.md` mis-named and `docs/ARGFORM.md` half-corrected

`nystromformer`'s wall was `aten.convolution.default: an asymmetric padding
[32, 0] is not implemented`. `docs/ARGFORM.md` §1 already found the first error:
...
`docs/ARGFORM.md` then classified it as *"a genuine backend limitation, not an
...
the same move `docs/RNN.md` §2 made for `conv1d(padding='same')` with an odd
total, one rank up:
...
not close, so the two halves of `docs/ARGFORM.md` §1's finding stay separable.
...
`docs/ARGFORM.md`'s own warning applies to whoever lands it: upstream accepts
