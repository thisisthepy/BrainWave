...
docs/ARCH200.md §2 named three operators worth six of its twenty-seven blocked architectures:
...
## 1. `conv1d` — a NAME, and it has been spelled since docs/ARCH20.md
...
cases since docs/OPS4.md; and `bootstrap.py` has bound `torch.conv1d` to it since
docs/ARCH20.md (`bootstrap.py:8883`). docs/ARCH200.md's `missing_shim_name torch.conv1d` row
...
docs/TAIL3.md (`view_as` is not `reshape_as`) and docs/VOICE3.md (`col2im` is not `im2col`'s
inverse) both record.
...
docs/GLU.md §2 recorded this op's schema and flagged docs/DEMAND8.md's three bicubic traps as
*"likely but unverified"* for it. Verified, one at a time, against upstream:

| docs/DEMAND8.md's bicubic trap | does it transfer to `linear1d`? |
|---|---|
...
The same gap docs/GLU.md §1.1 recorded for `glu` and `tools/golden/reach_allow.json` already
...
function** rather than behind a second helper, because docs/VOICE3.md found the derivation
...
### 5.1 Counted the way docs/ARCH200.md §3.3 asks
...
documentation corrected 1 (docs/ARCH200.md's `torch.conv1d` row: a name, and already spelled)
deleted                 nothing
```
