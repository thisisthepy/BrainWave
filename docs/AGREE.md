...
`docs/ARCH100.md` and `docs/ARCH200.md` both end on the same sentence, and neither acted on it:
...
for agreement — the twenty of `docs/ARCH20.md` plus the six of `docs/ARCH26.md`. This round ran
...
in §3 is float32 accumulation over depth, exactly as `docs/DEMAND8.md` §1.5 concluded for
...
answers scored against it — `docs/DEMAND8.md` §1.4's control, generalised from one model to 263.
...
GELU differ for reasons `docs/MPSFWD.md` already recorded.
...
truth for that same output**. `docs/DEMAND8.md` measured that ratio at ~1.2 for `mobilenet_v2`
...
model `docs/DEMAND7.md` §3 ranked as a "correctness bug" and `docs/DEMAND8.md` struck; this round
...
`docs/DEMAND8.md` §1.3's technique, reused. Upstream records the input and output of every leaf
...
these tables, and they are the same ops `docs/DEMAND8.md` §1.3 found at 2.4 ulp and
`docs/DEMAND1.md` predicts for a differently-associated but equally valid arrangement of the same
formula.
...
**BatchNorm running statistics.** Fresh `BatchNorm` has `running_var=1`, and `docs/DEMAND8.md`
...
## 7. Limits — `docs/ARCH200.md` §5's, restated, plus this round's own
...
binary and reports a plausible but wrong number (`docs/ARCH100.md` §8).
