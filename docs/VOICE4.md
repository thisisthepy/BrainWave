...
Seven rounds of `docs/VOICE.md` and `docs/VOICE3.md` closed operators that speech models *tripped
...
3. **It is deterministic.** No `generate`, no sampler, no temperature. `docs/AGREE.md` had to
...
| rejected | why not |
|---|---|
| **Vocos** | The obvious alternative vocoder, and `docs/VOICE.md` §2.1 already ran its construction on real weights. Its forward is an **iSTFT**: `torch.polar`, `view_as_complex`, `fft.irfft`. VOICE.md §3 measured that as a **candle-level** gap — `candle_core::DType` has no complex variant at all. Not reachable by adding kernels, so it cannot answer "did it reach the end". |
...
| **Parler-TTS** | Still blocked at `aten.detach_.default`, which `docs/INPLACE.md` refuses **on purpose**. Running it would mean reversing a deliberate design refusal, not closing a gap. |
...
This is `docs/ARCH100.md`'s lesson with a different variable. That round measured missing *names*
...
`docs/META.md` §7.4 had in fact already named both — its table of what meta still cannot reach
...
Not chosen. `docs/AGREE.md` §2's method, applied to this model rather than to its population:
...
and a kernel is the whole lattice. `docs/VOICE3.md` §2 made the same argument about `i0` needing a
