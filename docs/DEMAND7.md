...
This round re-measures the 19 architectures from docs/DEMAND.md and docs/DEMAND6.md, and then widens the set with 10 new architectures and tasks (including a decoder-only MoE, vision transformers with detection/segmentation, a multimodal model, state-space/recurrent models, and `.generate()` across seq2seq).
