...
`docs/ARCH200.md` left 27 architectures blocked behind 16 operator names. This
...
`docs/TAIL3.md` §1, and for the same kind of reason: the error names the wrong
operand.
...
nobody writes?** `docs/COMPLEX2.md` is the precedent for the standard: `tensor()`
...
### 1.3 So the sizing is `docs/TAIL3.md`'s, unchanged, and now with a
...
`docs/ARCH200.md` and `ARCH100.md` both measured that missing *names* outnumber
...
had already recorded the trap twice: `docs/DEMAND8.md` found that
`index_add_`'s negative indices do **not** wrap while `index_put_`'s do, and
`docs/TAIL3.md` §3 found `scatter_reduce`'s `sum` and `index_add_`'s
...
list — which is why `docs/VOICE3.md` saw the line number move *backwards*, from
710 to 705.
...
* **It does not need anything added for capture or the tape.** The name ends in
  ...
  its replay refused. `docs/BACKWARD8.md`'s `forgive_own_write` is the
...
`docs/ARCH200.md` filed `aten.embedding.default.unsupported` under "backend
...
data for the next batch, in `docs/ARCH200.md`'s sense.
...
* **`device.rs::MPS_HOST_READBACK_OPS` is unchanged, and that is a result
  ...
  which follows helpers only one level by name — the gap `docs/VOICE3.md`
...
`docs/TAIL1.md` measured the same distinction for the `and` pair and found the
...
checked rather than assumed, the way `docs/BIND3.md` checked `torch.im2col` and
