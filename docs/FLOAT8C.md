...
`docs/FLOAT8B.md` enumerated all 197 ops against upstream 2.13.0 and closed 114
...
`docs/FLOAT8.md` diagnosed the hang correctly: `candle-core` 0.11.0's
...
What `docs/FLOAT8B.md` §4.1 did **not** establish is how far that poison reaches.
...
reading float8 results by this same widening since `docs/FLOAT8B.md` §5.1 — the
...
`docs/FLOAT8B.md` table D was produced by a generic recipe that synthesises
...
This is the same shape as `aten.matmul.default` in `docs/FLOAT8B.md` §2.1 — a
...
### C. The three paths from `docs/FLOAT8.md`, which were never aten ops
...
upstream refuses too**, transcribed kernel by kernel (`docs/FLOAT8B.md` §2),
...
| 4 | that the 114 transcribed refusals are still exactly right | they were re-run as a group by the suite, not re-enumerated op by op against upstream. `docs/FLOAT8B.md` §2 is the enumeration |
