...
`docs/EXPORT5.md` §0 is the reason, and it is a measurement rather than a
preference:
...
on it, and it would fail on the first real architecture. `docs/DYNAMO.md` §18
reached the same conclusion from the other side for `torch.compile`, and
`docs/CAPTURE.md` §9 lists trace serialisation as *absent* — a captured trace
...
`docs/QUANT2.md` §3 already settled this shape for `quantize_`, and cited the
...
as every failure `docs/NPU2.md` records, one layer up.
...
`docs/NPU.md` §2's distinction, kept: **executed** and **structurally
...
which is the reason `docs/NPU2.md` §3.1 gives for `nnapi_runner.c` being a
...
This is the section the round exists for. `docs/NPU2.md` is this project's
record of paying for the mistake twice — the CoreML models `docs/NPU.md`
...
This is exactly the argument `docs/NPU2.md` §2 makes with its third row — two
...
instead of inherited, which is the failure mode `docs/DOCWATCH.md` exists for.
...
`docs/AGREE.md` §2's rule, applied to one module instead of 263
...
`docs/AGREE.md` §3 found across 285 architectures: **below 1 means the
...
* with `kHtpFp16`, the honest tolerance is a **float16** tolerance. `docs/NPU2.md`
...
* with `kHtpQuantized` — the default — the numbers are **quantised**, the
  ...
  `docs/QUANT2.md` §2.1 is the precedent: quantisation cannot go on this
...
on one run,** exactly as `docs/NPU2.md` §1.1 found for the Neural Engine, and
...
on Qualcomm silicon at the cost of an offline Linux build. `docs/NPU2.md` §5
...
This is `docs/EXPORT5.md` §11's finding repeating — the uncaught nullification
...
each skip line has to name the missing thing (docs/VULKAN3.md §6.1: a skip
...
* **No QNN artefact exists.** §1.3. Everything from `to_edge_transform_and_lower_to_qnn`
  ...
  **unknown**. They may be poor: QNN's 112 op builders are not the same set as
  XNNPACK's, and `docs/REFOLD.md` §1.1 is this project's record of assuming a
...
* **The quantised path was not built.** `kHtpQuantized` is QNN's default and
  ...
  axis. `docs/QUANT2.md` §2 is what that axis has to look like.
...
* **No whole model, only one submodule.** `model.layers.0.mlp` is the smallest
  ...
  the KV cache, which `docs/CAPTURE.md` §9 already names as the wall mutation
  hits first.
* **No timing, anywhere.** Three other rounds were building on this machine
  and load average was above 8 on 8 cores throughout. `docs/MPS.md` and
  `docs/VULKAN3.md` both record why a throughput number measured like that is
...
local SmolLM2-135M the checkpoint tests skip saying so. `docs/VULKAN3.md` §6.1 is why every skip line
