...
`docs/FEDERATED2.md` landed N rounds of it over `Delta.re_snapshot`. Both
...
`torch.equal`, not a tolerance, for `docs/FEDERATED.md` §2.1's reason: the
...
`docs/FEDERATED.md` §8 listed "a rank that dies mid-round was not tested",
...
disagreements `docs/FEDERATED.md` §3 found. And a rule as ordinary as "sample
...
world but 1 and 2 (`docs/TRANSPORT.md` §3). At two ranks every subset that is
...
| optimiser reset between rounds | unchanged from `docs/FEDERATED2.md` §4 — the state carries forward and nobody has measured the difference |
