...
> **Superseded in part by `docs/numerics/FLOAT8C.md` (round 3).** Tables **D** and **E**
...
> computes; `docs/numerics/FLOAT8C.md` §2 has the re-measurement. Everything else here —
> §2's 114 transcribed refusals above all — still holds.

`docs/numerics/FLOAT8.md` turned three hangs into named refusals and then recorded a wider
...
assumed, for the reason `docs/numerics/PROMOTE.md`'s last section exists. The **kernel name
...
direction — the same trade `docs/numerics/FLOAT8.md` made for `tolist`, `item` and the
...
`aten.pow.Scalar` would be a lie, because upstream implements it:
    ...
    terminate); upstream computes this -- docs/numerics/FLOAT8B.md 4.1

Fixing them properly means patching `candle-core`, which is a vendored-dependency
change and a different round. Recorded and refused, the way `docs/architectures/DEMAND1.md` did
...
`F8E4M3` matmul). These are `docs/numerics/FLOAT8.md`'s own refusals plus two pre-existing
...
**That reason is stale and was already stale when `docs/numerics/FLOAT8.md` was written** —
...
`.tolist()` for `float8_e4m3fn`** (docs/numerics/FLOAT8.md, and still true -- the
...
the other -- because docs/design/TORCH_C.md §1's point is that those two are not
interchangeable.
...
`docs/architectures/DEMAND1.md`'s rule -- a verification that cannot fail is not a verification
...
green. The block is now last in the file. That is `docs/architectures/DEMAND1.md`'s rule again
...
### 7.3 `docs/numerics/FLOAT8.md`'s path table still holds

Re-run against this build, both sides. Every row is as `docs/numerics/FLOAT8.md` recorded
...
which is precisely the shape docs/numerics/PROMOTE.md's last section exists to prevent.
