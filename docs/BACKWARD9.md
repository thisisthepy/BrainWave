...
`docs/BACKWARD7.md` §6 is a list of seven things standing between an eager recorder that works and
the spelling every tutorial uses. `docs/BACKWARD8.md` closed the two on it that turned out to be
...
| **`allow_unused`?** | **Closed, and closed in the place that makes both defaults reachable.** `docs/BACKWARD7.md` §6 called ours an "opposite default"; it is upstream's now. Where the fix went is the content -- §4 |
...
| **Did the forward move?** | **No.** The three `docs/BACKWARD7.md` §7 digests reproduced from the recipe in the tree. §7 |
...
`docs/FEDERATED3.md`'s central-oracle pattern, and it is why nothing below is a literal: a hardcoded
trajectory is a claim about 2.13.0 that nothing re-checks, and `docs/AUDIT.md` found that shape of
...
* **The parameters are initialised arithmetically, not from a seed.** The two interpreters do not
  ...
  `docs/BACKWARD8.md` §3 met the same wall from the other side, where a mask had to be *shared*
  rather than avoided.
...
tape to disagree, which is the property `docs/BACKWARD7.md` §2 bought by splitting `tape::backward`
...
`docs/BACKWARD7.md` §6 called this "the piece with real design content" and said the interaction
...
`docs/BACKWARD8.md`'s guard poisons a tape when a write lands on a storage it holds. It cannot fire
...
`docs/BACKWARD7.md` §6 recorded that `_eager_backward` returns `None` for a leaf no gradient reached
...
is the evidence rather than an argument about compile inputs. `docs/BACKWARD7.md` §9.1's recipe is in
...
All three equal `docs/BACKWARD8.md` §5's and through it `docs/BACKWARD7.md` §9.1's,
`docs/BACKWARD6.md` §7's, `docs/BACKWARD5.md` §6.5.1's and `docs/BACKWARD4.md` §6.2's -- a six-round
...
1. **`test_tensor_backward_still_refuses_even_though_an_eager_graph_exists`** →
   ...
   `torch.autograd.grad`. The `docs/BACKWARD7.md` DOCWATCH marker follows the test to its new name
...
| # | not established | why |
|---|---|---|
| 1 | **That a real model trains through this.** | §1's loop is `nn.Sequential(Linear, Tanh, Linear)` on 5 rows. `docs/BACKWARD8.md` §2.1 differentiates SmolLM2-135M through `_eager_backward`, but nothing here runs an `optimizer.step()` loop over a transformer, and `docs/BACKWARD8.md` §8 row 2's missing convolution derivative still blocks the vision half |
...
| 4 | **Any per-dispatch or per-backward cost.** | Six agents were on the machine. `docs/BACKWARD7.md` §10 row 1 stands unchanged, and this round reports no time at all |
...
| 6 | **`backward()` across a capture-region boundary.** | `docs/BACKWARD7.md` §10 row 7, unchanged. `eager_record` returns early while a region is open, so those ops are absent from the eager graph, and now that `.backward()` exists the question is askable and still unasked |
