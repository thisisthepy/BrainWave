...
`docs/TAIL1.md` and `docs/TAIL2.md` closed the top of `docs/ARCH100.md`'s ranking. Each closure
...
`einsum` (`longt5`) — plus the item `docs/ARCH100.md` classified not as a gap but as a **backend
...
`docs/SETITEM.md` proposed a `contiguous()` as the plausible fix. **The operands were already
...
`docs/ARCH100.md` established that missing **bindings** outnumber missing **kernels** 49 to 22 in
...
`index_add_` accumulates **at the receiver's dtype, per step** (docs/DEMAND8.md measured it).
...
`docs/SCATTER.md` said so and this restates it because the names are one character apart.
...
divergence shape `docs/VULKAN2.md` requires be unrepresentable rather than merely unused, and the
same gap `docs/VIEWS.md` §6.4 already carries for `slice.Tensor` with step > 1 and for
`view.dtype`.
...
`docs/DOCWATCH.md` exists to prevent.
