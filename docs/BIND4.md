...
the binding was written** — `docs/BINDINGS.md`'s `mish` is what happens when that check is
...
| 4a. `torch._C._nn.avg_pool2d` | Yes, already bound (docs/BIND2.md) | confirmed still bound |
| 4b. `nystromformer` | N/A | `aten.convolution.default`'s asymmetric-padding refusal — candle backend limitation, `aten.rs`, **not actionable here** (re-confirms docs/ARGFORM.md §1) |
...
round (`docs/OPS4.md`). `bootstrap.py`'s `conv1d` composite already handled every branch except
this one, which it refused by name (docs/RNN.md §1).

The fix is the four-line transcription docs/RNN.md §1.2 measured and could not land (RNN was not
...
compared, and bit-compared against upstream by docs/RNN.md §3 — the gap was one `_install_nn`
line, shaped like `upsample_nearest1d`'s one line above it, and specifically not landed there
because `bootstrap.py` was not that round's file either (docs/RNN.md §3.1).

**The discriminator is the fourth argument's TYPE, not the arity** — docs/BIND3.md §3.1's exact
...
values, seven output widths (crossing the FUSED-multiply-add ULP boundary docs/RNN.md §3
...
upstream before being added, per docs/ARGFORM.md's own bar.
...
**Measured to be general, not `zeros`-specific**, the same way docs/ARGFORM.md §2 requires before
...
measurement for the others, exactly the silent-divergence trap docs/ARGFORM.md §2 names.
...
past this point from this file** — recorded rather than left unattributed, per docs/TAIL4.md
...
`torch._C._nn.avg_pool2d` is bound (`docs/BIND2.md`), confirmed present in `_shim_nn_implemented`
...
candle's conv2d takes one value per argument, not one per axis`. This is `docs/ARGFORM.md` §1's
...
Handed off by the coordinating session mid-round: the complex round (docs/COMPLEX3.md, merging
...
work — right about the operators, wrong about the door. `docs/COMPLEX3.md` §6.2 carries the
...
* `module._fft.fft_fftn` — the decomposition docs/COMPLEX3.md §6.1 read off a
...
**No `aten.fft_fftn.default` kernel was added**, deliberately, per docs/COMPLEX3.md §6.3's own
...
absolute difference 7.15e-07 over 256 outputs — is docs/COMPLEX3.md §6.1's own measurement, from
