...
Per docs/BINDINGS.md's warning, **every kernel below was confirmed present in
...
been golden-compared since `sew_d` -- `docs/TAIL3.md` §7 already established
...
separate `.vec` overload to choose between (the arity trap docs/BINDINGS.md
...
`aten::einsum` is `CompositeImplicitAutograd` (docs/TAIL3.md §7 already
...
than the measured call, which is docs/BINDINGS.md's `upsample_nearest2d`
...
(docs/PAD.md §4); this round re-confirmed all six in `_aten_implemented()`
before touching `pad`'s dispatch.

**What was missing:** exactly the patch docs/PAD.md §5 wrote out in full,
...
design docs/PAD.md §5 and the entries' own `reason` text both call for. Golden
itself (`tools/golden/compare.py`) is unmoved: these six entries gated
*reach*, not the kernel comparison, and no kernel changed.

Per docs/PAD.md §5's own caveat: this is the step that makes "four speech
...
`overloads.json` row (confirming docs/TAIL3.md §8's own note). So per this
...
* **`var` is `std`'s square** (docs/TAIL3.md's own framing) -- also absent,
...
* **`linalg_vector_norm`** (`aten::linalg_vector_norm`, present, has an
  ...
  is exactly the kind of divergence docs/PAD.md §3's `circular` refusal and
...
`docs/ARCH100.md`/`docs/TAIL3.md`'s territory boundary, not this round's, and
...
docs/TAIL3.md §8 already recorded `gemma3n_text` itself moving from `erfinv`
