...
## 1. The headline: the CoreML models docs/NPU.md executed ran on the **CPU**

`docs/NPU.md` §2 recorded the `.mlpackage` as **executed** — compiled by macOS,
...
to which one it picked. Read for all three of docs/NPU.md's float32 graphs
...
### 1.1 And the reason is the flag docs/NPU.md was right to set
...
The Neural Engine is float16 hardware. So the two claims docs/NPU.md wanted are
...
None of this is a defect in docs/NPU.md. It is a distinction that document did
...
2.0e-04 is float16, and it is the same order docs/NPU.md measured for the
...
these numbers were taken, and docs/MPS.md and docs/VULKAN3.md both record why a
throughput number measured like that is worse than no number.

## 3. NNAPI, executed — and by which driver

docs/NPU.md §2 put the NNAPI blob under **structurally validated** and said
...
### 3.4 The whole model — docs/REFOLD.md §4's deliverable, executed
...
> **docs/REFOLD.md §4's line moves.** That section carries a blockquote saying
> "Still structurally validated, not executed. There is no NNAPI runtime on a
> Mac; docs/NPU.md §2 draws that line and nothing here moves it." This does.
...
convolution in docs/NPU.md §4 whose fault injection was the identity.
...
docs/REFOLD.md left a standing warning — it measured `mobilenet_v2` getting
...
| **claim corrected** | docs/NPU.md's executed CoreML claim is a CPU claim; docs/REFOLD.md §4's "not executed" no longer holds |
...
CoreML tests skip saying so. docs/VULKAN3.md §6.1 is why both skip lines name
...
app** — docs/VULKAN3.md §4's precedent for using what is already on disk
