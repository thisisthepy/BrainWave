...
세 번째 줄이 이 문서의 새로운 내용입니다. 나머지 둘은 `docs/VULKAN.md` 가 이미 답해 두었고,
저는 그것을 재확인만 했습니다.
...
`docs/VULKAN.md` §4.2 의 판정을 이번에 독립적으로 다시 확인했습니다
...
`docs/VULKAN.md` §4.1 은 호스트 macOS 에서 프로브가 이렇게 죽는 것을 기록했습니다:
...
`docs/BACKWARD5.md` 의 교훈대로 **이 숫자들은 전부 세거나 실행한 것**입니다 (396, 20, 21, 203, 2,
...
- **`rust/torch_c` 를 변경하지 않았습니다.** 이 라운드의 변경은 `docs/VULKAN2.md` 하나입니다.
