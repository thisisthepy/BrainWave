...
`docs/ARCH100.md` swept 297 architectures and found thirty-one operators left, twenty of which
...
`docs/ARCH100.md` established that in this tail missing **bindings** outnumber missing **kernels**
...
`docs/ARCH100.md`'s own classification is worth restating because it does not line up with this
...
`docs/PRIMS.md` §6 found `prims.transpose` **stricter** than `aten.permute` — it refuses a
...
`docs/DEMAND8.md` found three separate traps in `upsample_bicubic2d`, each discovered by running
...
**misnamed the kernel** for the three it refuses. `docs/PROMOTE.md`'s rule — transcribe the
...
`docs/ARCH100.md` counts `broadcast_tensors` as one architecture (`gemma3n_text`). The reason to
lead with it is `docs/BACKWARD9.md` §1, which had to spell its criterion `((o - t) ** 2).mean()`
...
raising stub in `bootstrap.py`. That is the second wall, of exactly the shape `docs/DEMAND8.md`
recorded for `new_empty` (`torch.maximum` behind it). `docs/BACKWARD9.md`'s hand-spelled
...
have, which is exactly what `docs/SPELLINGS.md` refuses.
...
(`docs/PRIMS.md` §6, `docs/DEMAND8.md`).
...
than a failure — `docs/DEMAND8.md` records the same shape for `new_empty` (`torch.maximum` behind
