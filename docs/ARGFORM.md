...
| `torch.ops.aten.adaptive_avg_pool2d.default(x, 2)` | **no**, raw aten op refuses | left refusing — confirmed still refuses after the fix (docs/FIXES.md §3's own trap) |
...
docs/FIXES.md §3 already measured and recorded where this belongs: `F.adaptive_avg_pool2d`'s Python
...
The fix is exactly the one door docs/FIXES.md named: `bootstrap.py`'s `adaptive_avg_pool2d`
...
the exact SILENT DIVERGENCE docs/FIXES.md §3 measured and reverted, pinned so it cannot come back
through this door.
...
(docs/GLU.md) with the binding deliberately left out, exactly the same shape as this round's own
...
shared wall (docs/ARCH100.md) is gone at the binding layer. That scratch worktree was discarded
...
docs/SETITEM.md §2's patch (measured, applied, and reverted by a concurrent round for the same
...
promotion docs/SETITEM.md §2.1 named already flipping from `expect="c_error"` to a live pass:
...
**This is `tools/golden/cases.py`, out of this round's territory**, and docs/SETITEM.md already
...
docs/SETITEM.md's own verification (§7, "481 ok, 0 FAIL") did not appear to hit, most likely because
