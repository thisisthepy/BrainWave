...
docs/ARCH100.md §2 ranks `TensorBase.__setitem__` first among the operators that block the 82
...
So docs/ARCH100.md's own caution is stronger than it put it. It noted that the 13 are "dominated by
...
rule were already there (docs/VIEWS.md §6); mixed basic-and-advanced indexing is still refused by
...
docs/VIEWS.md §4 with candle's `same_storage` as an oracle: `slice.Tensor(x, 0, 5, 2)` holds an
...
built now build**, which is the part docs/ARCH100.md §4 says is worth separating: "cannot even be
...
* **`TensorBase.view_as`** (2 arch) is a spelling gap of the docs/DEMAND8.md §2.1 kind — one method
  on `PyTensorBase`, no kernel, since `view` is already there. It is not in docs/ARCH100.md's
  ranked list at all, because nothing reached it before.
* **`TensorBase.index_select`** (1 arch here) is already **rank 4** in docs/ARCH100.md, blocking 5
...
* **`aten.matmul.default` / `MatMulUnexpectedStriding`** (5 arch) is not a missing operator. It is
  the `backend_limitation` class docs/ARCH100.md §2.1 kept separate — candle refusing a
...
Read against docs/ARCH100.md's roadmap: the head of the ranked list is worth less than 13 and more
...
(`x[t] = v`, measured in docs/VIEWS.md §2). But a stepped write is not one of those upstream; it is
...
**Negative slice bounds resolve; they are not the index-wrap question.** docs/DEMAND8.md warns by
...
`__setitem__` is an in-place write, so docs/BACKWARD8.md's storage-version guard and the eager
...
silently wrong gradient — which is the convention docs/INPLACE.md records. The new form joins that
...
all, and it holds because `index_put_` goes through `write_back` (docs/VIEWS.md §6): a view taken
...
That is the whole reason the top-ranked blocker cost no arithmetic: docs/ARCH100.md's split of
...
One further test reads `docs/SETITEM.md` and checks that the anchor §2's patch keys on still appears
