...
blocks PEP 523 — docs/COMPILE.md), so had the mechanism been routed through it, this
...
That second, dependency-free quantizer is what docs/QUANT2.md §3 was pointing at when it
...
answer in docs/NPU2.md: the runtime's own statement of fact, queried *after*
...
answers per operation, which is how docs/NPU2.md caught a partially-offloaded model.
...
Row 4 is docs/NPU2.md §3.5's control, and it nearly shipped broken here in the same
...
| `torch.compile` / dynamo | `IntelNPUUnsupported`, pointing at docs/COMPILE.md. Permanent. Unlike the archived library, torchnative does not offer the optional dynamo backend at all |
...
| `REFUSED: ... EXECUTION_DEVICES = ['NPU', 'CPU']` | Partial offload — part of the graph runs on the CPU. Refused. This is the case docs/NPU2.md records going unnoticed on CoreML. |
...
entire lesson of docs/NPU2.md, and it is why this module raises where
...
5. **No quantization.** `quantize_` refuses. §1.4 shows the dependency-free scheme is
   ~35 lines and that docs/QUANT2.md §3 already wants it; `int4`'s nibble packing would
...
docs/VULKAN3.md §6.1 is why every skip line names the missing thing: a skip with a false
