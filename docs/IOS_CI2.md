...
`docs/IOS_CI.md` answered "where does the runner get an `arm64-iphonesimulator`
...
Before writing a single step, two claims from `docs/TARGET_PYTHON.md` needed
...
- **Does the wheel-selection glob match what `pip download` actually
  produces?** `docs/IOS.md` records a real filename,
...
2. **Cache the iOS-simulator CPython** (`actions/cache@v4`) — keyed on the
   ...
   `docs/TARGET_PYTHON.md` §1) plus the Xcode version string. This is the step
...
3. **Build the arm64-iphonesimulator CPython** (only on cache miss) — follows
   ...
   - `make && make install`. `--enable-framework=DIR` with an absolute `DIR`
     ...
     `DIR` — the same top-level shape `docs/TARGET_PYTHON.md` describes for
...
- **The `./configure`/`make`/`make install` invocations for both the host and
  ...
  was made by hand (`docs/TARGET_PYTHON.md`), not from a script that can be
...
- **The YAML parse check is syntactic only.** It confirms the file is valid
  ...
  brief.
- **Whether `macos-15` ships an Xcode new enough.** `docs/TARGET_PYTHON.md`
...
Unchanged from `docs/IOS_CI.md`: **the device rung is untouched.** A runner
