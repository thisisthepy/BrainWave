...
> **Read `docs/QUANT.md` §2.1, `docs/QUANT2.md` §3 and `docs/DTYPE.md` §6 first.**
...
`docs/FLOAT8C.md` recorded that this crate's `Cargo.toml` has **no `[patch]` section**
...
same property `docs/DTYPE.md` §6.3 noted for this shim's own `Repr` enum, and it holds
...
The `matmul` line is the point `docs/QUANT2.md` §3 makes, confirmed rather than
...
the same precaution `docs/FLOAT8B.md` §1 needed, kept because it costs nothing. **No
op hung.**
...
The first three are **`docs/QUANT2.md` §3's point 3, measured**. candle has no integer
...
1. **It requires forking a pinned crates.io dependency into the shipped build.**
   ...
   deliberately has none today (`docs/FLOAT8C.md`, re-verified §1.2). That reaches
...
- **Any performance number.** Five other agents were running throughout
  (`load average 21.49` at the start, `7.65` at the lowest); per `docs/QUANT.md` §1
