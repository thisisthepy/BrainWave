...
> **결론 먼저.** `docs/perf/PERF.md` 가 Apple 에서 회수한 **7 배는 안드로이드로 전이되지 않습니다.**
...
`docs/perf/PERF.md` 에도 다른 문서에도 없습니다. 아래 §2 는 그 분해를 인용하지 않고 다시 잽니다.
...
즉 `docs/perf/PERF.md` §3 이 기록한 "feature 플래그 하나로 7 배가 1 배" 의 정체는 **Accelerate 가
...
> **배송 빌드는 바뀌지 않았습니다.** 근거는 `docs/devices/DEVICE.md` §5.2 이고, Apple ↔ 안드로이드
...
| `_softmax` | 이 문서가 다루지 않는다. `docs/perf/PERF.md` §4 참조 |
| 4 M 이 실기 안드로이드에서도 맞는 값인지 | **미확인.** M1 에서 골랐고, 기기에서 회귀하지 않음만 확인했다 |

---

# 10. 이번 세션의 다섯 최적화 — 안드로이드로 전이됐는가

`docs/bindings/BIND.md` §7 은 바인딩 플랜(최적화 1)을 기기에서 쟀지만(view 2.75x, transpose 2.51x,
add 1.80x), 같은 세션에 착지한 나머지 넷 — 디스패치 키워드 관례(`docs/design/DISPATCH.md`),
`pow` 를 곱셈으로 바꾼 것(`docs/numerics/SEQLEN.md` §2-3), 전치를 통과하는 bf16 GEMM 확장
(`docs/perf/DTYPE_PERF.md`), `amax`·스케일-마스크 융합·5.25x 어텐션 복사(`docs/numerics/SEQLEN.md` §7-8)
— 는 **호스트에서만 측정됐다.** 이 절은 그 넷을 기기(`emulator-5554`, API 26 arm64-v8a)에서 잰다.

**기준선.** `dist/` 의 안드로이드 휠 `torchnative-{0.0.2a0..0.0.6a0}-...-android_21_arm64_v8a.whl`
을 풀어 `_C.abi3.so` 를 `nm -C`/`strings` 로 대조한 결과, 각 최적화가 도입된 커밋의 새 심볼이
휠 사이에 다음과 같이 갈렸다(전부 `nm`/`strings` 로 직접 확인, 표는 존재 개수):

| 휠 | `_ArgPlan`(#1) | `interned_name`(#2) | `pow_square_fast_path`(#3) | `widen_gemm_operand`(#4) | `amax_keepdim`(#5a) | `scale_and_causal_mask`+`transposed_contiguous`(#5b/c) |
|---|:--:|:--:|:--:|:--:|:--:|:--:|
| 0.0.2a0 | 없음 | 없음 | 없음 | 없음 | 없음 | 없음 |
| 0.0.3a0 | **있음** | 없음 | 없음 | 없음 | 없음 | 없음 |
| 0.0.4a0 | 있음 | **있음** | 없음 | 없음 | 없음 | 없음 |
| 0.0.5a0 | 있음 | 있음 | **있음** | **있음** | **있음** | 없음 |
| 0.0.6a0 (= `d2cafb9`, 이 브랜치의 develop) | 있음 | 있음 | 있음 | 있음 | 있음 | **있음** |

즉 릴리스 휠 자체가 다섯 최적화의 착지 순서를 그대로 기록하고 있고, 소스에서 다시 빌드하지
않고도 각 최적화 전후를 격리할 수 있다. `torch/` 트리의 순수 파이썬 부분은 `diff -rq` 로
...
매 구간(§10.1~10.5) 전후로 확인했고, 네 구간 여덟 번의 측정 모두 1 분 평균 1.71~1.96,
5 분 평균 2.35~2.98, 15 분 평균 3.15~3.60 사이였다 — 다른 세션의 빌드가 간헐적으로 도는
상태였지만 측정 구간 자체에서는 급격한 스파이크가 없었다. (측정 시작 전, 휠을 살펴보던
탐색 단계에 1 분 평균이 한 차례 5.83 까지 오른 적이 있으나 그건 타이밍 루프 바깥이다.)

## 10.1 최적화 #2 — 디스패치 키워드 관례 (`docs/design/DISPATCH.md`, `e39c65e`)

휠 `0.0.3a0`(old, 키워드 언패킹 경로) 대 `0.0.4a0`(new, `interned_name` 인터닝 경로).
`docs/bindings/BIND.md` §7.2 와 동일한 모양 `(1,6,9,64)` f32, 워밍업 200, 2 만 호출씩 5 블록의 최솟값.
3 회 번갈아, µs/call:

| | old (0.0.3a0) | new (0.0.4a0) | 배수 |
|---|---:|---:|---:|
| `.view(1,6,576)` | 2.6736 (2.6736/2.7229/2.6766) | 2.2398 (2.2657/2.2883/2.2398) | **1.19×** |
| `.transpose(1,2)` | 2.0373 | 1.7158 | **1.19×** |
| `t + t` | 2.6159 | 2.3559 | **1.11×** |
| `.unsqueeze(0)` | 1.8190 | 1.5515 | **1.17×** |

**대조군**(0.0.4a0 를 두 라벨 A/B 로 중복 측정): view 1.0015×, transpose 1.0069×, add 1.0171×,
unsqueeze 1.0317×. 대조군이 최대 3.2% 까지 흔들려 `docs/bindings/BIND.md` 의 대조군(1.03~1.04x)보다
조금 더 넓지만, 관측된 효과(11~19%)가 그보다 여전히 3.5~12 배 커서 잡음으로 설명되지 않는다.

**전이됐다, 그리고 호스트보다 크다.** 호스트(`docs/design/DISPATCH.md`): view 1.100×, transpose
1.153×, add 1.073×, unsqueeze 1.118×. 기기: view/transpose 1.19×, add 1.11×, unsqueeze 1.17× —
네 지표 모두 호스트보다 크거나 같다. `docs/bindings/BIND.md` §7 이 예상한 대로("느린 인터프리터가
파이썬 층의 몫을 키운다") 다.

## 10.2 최적화 #3 — `pow` 를 곱셈으로 (`docs/numerics/SEQLEN.md` §2-3, `59935ef`)

휠 `0.0.4a0`(old) 대 `0.0.5a0`(new, `pow_square_fast_path` 추가). 실제 RMSNorm 모양
`[1,S,576]` f32 연속, `pow(2)` 대 `x*x`. 워밍업 10, 15 회 최솟값, ms (3 회 번갈아 최솟값):

| S | old `pow(2)` | new `pow(2)` | 배수 | new `x*x` (대조) | old `x*x` |
|---:|---:|---:|---:|---:|---:|
| 128 | 0.48000 | 0.02846 | **16.87×** | 0.02817 | 0.02813 |
| 512 | 2.04608 | 0.10687 | **19.15×** | 0.10617 | 0.10813 |

**대조군**(0.0.5a0 를 두 라벨로): `pow2_S128` 0.9900×, `pow2_S512` 1.0047×, `x*x` 양쪽 다
0.98~1.03× — 1.5~2% 안쪽. `x*x` 자체는 old/new 사이에서 거의 움직이지 않는다(S=128: 0.02813
대 0.02817, S=512: 0.10813 대 0.10617) — 고쳐진 것이 `pow(2)` 의 6-패스 경로뿐이고 곱셈
경로 자체는 건드리지 않았다는 §3(호스트)의 주장과 일치한다.

**전이됐고, 미시 벤치 자체의 배수가 호스트보다 크다.** 호스트(`docs/numerics/SEQLEN.md` §2.3, S=512)의
`pow(2)`: 상류 0.0314ms, 우리(old) 4.2598ms, 우리(new, `x*x`) 0.0271ms — old/new 로 다시 읽으면
**157×**. 기기의 old/new 는 19.15×(S=512) 로 호스트보다 작다. 다만 이 비교는 절대 규모가
전혀 다른 두 기계(호스트가 훨씬 빠른 CPU) 위에서 잰 값이라 **직접 대조할 근거가 약하다** —
호스트/기기 배수 차이를 "전이가 약하다"는 결론으로 쓰지 않는다. 방향(수십 배 개선)과 부호는
분명히 맞고, 모델 수준 재구성(§10.5)에서 다시 본다.

## 10.3 최적화 #4 — bf16 GEMM 확장을 전치 뒤로 (`docs/perf/DTYPE_PERF.md`, `c8ed26f`)

같은 휠 쌍 `0.0.4a0`/`0.0.5a0`(`widen_gemm_operand` 추가). `nn.Linear` bf16 순전파, 실제
SmolLM2-135M 모양. f32 는 변하지 않아야 한다는 것이 이 최적화 자체의 주장이므로 대조로 같이 쟀다.
워밍업 5, 10 회 최솟값, ms (3 회 번갈아 중 최솟값):

| 모양 | old bf16 | new bf16 | 배수 | old f32 | new f32 | f32 배수(대조) |
|---|---:|---:|---:|---:|---:|---:|
| q/o_proj `[128,576]@[576,576]` | 1.80612 | 1.20817 | **1.49×** | 1.03771 | 1.03683 | 1.00× |
| gate/up `[128,576]@[576,1536]` | 4.84079 | 3.19104 | **1.52×** | 2.72612 | 2.72704 | 1.00× |
| down_proj `[128,1536]@[576,1536]` | 4.80708 | 3.17612 | **1.51×** | 2.69662 | 2.69488 | 1.00× |

**대조군**(0.0.5a0 두 라벨): bf16 세 모양 1.0016~1.0031×, f32 세 모양 0.9985~1.0004× —
1% 이내. f32 는 old/new 양쪽 다 사실상 동일(1.00×) — GEMM 확장이 f32 경로를 건드리지
않는다는 문서의 주장과 정확히 일치한다.

**전이됐다 — 방향은 맞지만 2.7× 가 아니라 1.5× 근처다.** 호스트의 2.60~2.68×
(`docs/perf/DTYPE_PERF.md` 결론부, SmolLM2-135M `S=128` 전체 prefill 469.4→180.7ms, 커밋 메시지는
463→173ms)는 **모델 수준 숫자**이고, 위 표는 단일 `nn.Linear` 호출의 배수다 — 같은 방향(50%
이상 개선)이지만 모델 전체 숫자와는 정의가 다르다. 단일 호출은 lm_head(가장 큰 GEMM, `[576,
49152]`)를 포함하지 않았다 — 반복당 시간이 길어 이번 라운드에서는 뺐고, 빠뜨렸다고 밝혀둔다.
§10.5 가 lm_head 를 뺀 채로 모델 형태에 더 가까운 합성 블록을 다시 잰다.

## 10.4 최적화 #5 — `amax`, 스케일-마스크 융합, 5.25x 어텐션 복사 (`docs/numerics/SEQLEN.md` §7-8)

세 하위 항목이 두 휠 경계에 걸쳐 있다: `amax_keepdim` 은 `0.0.5a0` 에서 이미 들어와 있고
(`fee8d17`), `scale_and_causal_mask`·`transposed_contiguous` 는 `0.0.6a0` 에서 처음
나타난다(`932a1ff`, `de7e624`). `scale_and_causal_mask` 는 `aten` 키가 없어 SDPA 를 통해서만
닿을 수 있다는 것을 조사 에이전트가 확인했으므로, 두 하위 절로 나눠 잰다.

### 10.4.1 `amax` (0.0.4a0 → 0.0.5a0)

`[1,9,512,512]` f32 에 `torch.ops.aten.amax.default(t, [-1], True)` 직접 호출(§10.2 와 같은
스크립트, 같은 라운드), 그리고 같은 크기로 SDPA 전체(`q(1,9,512,64) k/v(1,3,512,64)`, causal,
`scale=0.125`, `enable_gqa=True`). 워밍업 5, 10 회 최솟값, ms (3 회 번갈아 최솟값):

| | old (0.0.4a0) | new (0.0.5a0) | 배수 |
|---|---:|---:|---:|
| `amax` 직접 호출 | *구현 없음 — `NotImplementedError: aten.amax.default`* | 0.28975 | 절대비교 불가 |
| SDPA 전체, `S=512` | 25.05029 | 22.50025 | **1.11×** |

**대조군**: SDPA `S=512` 0.9915×(controlA/controlB), `amax` 직접 호출 0.9999× — 1% 이내.

`amax` 는 0.0.4a0 에 아예 구현이 없어(§0 심볼 대조 표) old/new 배수를 낼 수 없다 — 없음에서
있음으로 바뀐 것 자체가 결과다. SDPA 전체로 보면 **1.11×** 개선. 호스트(`docs/numerics/SEQLEN.md`
§7.6, `S=512`, 상류 대비): 2.007×→1.684×, old/new 로 다시 읽으면 463.73/389.01=**1.19×**.
**여기서는 기기(1.11×)가 호스트(1.19×)보다 작다** — `docs/bindings/BIND.md` §7 의 "느린 인터프리터가
전이 폭을 키운다"는 이 항목에서는 성립하지 않는다.

### 10.4.2 스케일-마스크 융합 + 어텐션 복사 (0.0.5a0 → 0.0.6a0)

같은 SDPA 전체(이제 amax 는 양쪽 다 있고, 융합·복사 수정만 차이), 그리고 복사만 단독으로
격리한 `k.transpose(2,3).contiguous()`. `S=512`, `1024`, ms (3 회 번갈아 최솟값):

| | old (0.0.5a0) SDPA | new (0.0.6a0) SDPA | 배수 | old 복사 단독 | new 복사 단독 | 배수 |
|---:|---:|---:|---:|---:|---:|---:|
| S=512 | 22.57642 | 20.25263 | **1.11×** | 0.23217 | 0.06317 | **3.68×** |
| S=1024 | 87.55800 | 76.45638 | **1.15×** | 0.45417 | 0.14542 | **3.12×** |

**대조군**: SDPA S=512 1.0062×, S=1024 1.0049×, 복사 S=512 1.0000×, S=1024 1.0224× —
2.3% 이내. 관측 효과(복사 212~268%, SDPA 11~15%)가 그보다 5~100 배 커서 잡음이 아니다.

**복사가 전이됐지만 5.25× 가 아니라 ~3.1~3.7× 다.** 호스트(`docs/numerics/SEQLEN.md` §8.3/8.6):
S=512 복사 단독 0.2123→0.0404ms = **5.25×**, S=1024 0.4232→0.0875ms = **4.84×**. 기기는
3.68×/3.12× — 방향은 같고 큰 개선이지만 두 길이 모두 호스트의 배수에 못 미친다. **SDPA 전체는
1.11~1.15×** — 이 커밋(`de7e624`)이 다른 여섯 커널도 함께 묶어 들여왔다고 조사 에이전트가
확인했으므로, 이 SDPA 배수는 융합(`932a1ff`)과 복사(`de7e624`) 둘의 합이고 호스트 쪽에 정확히
같은 경계로 잘린 수치가 없어 직접 대조하지 않는다.

## 10.5 합성 디코더 블록과 `nn.Linear` — 세션 전체의 누적 효과

`0.0.2a0`(다섯 최적화 전부 없음, §0 대조 심볼 표에서 확인)를 `0.0.6a0`/`d2cafb9`(develop,
다섯 전부 있음)와 비교한다. SmolLM2-135M 실제 HF 체크포인트는 기기에 없고(`/data/local/tmp`
에 캐시가 없음을 확인했다) 푸시하는 것도 이 작업 범위 밖이라 **올리지 않았다** — 대신 같은
모양(`hidden=576, heads=9, kv_heads=3, head_dim=64, intermediate=1536, S=128`)의 **합성
디코더 레이어 하나**를 무작위 가중치로 구성해 썼다. `torch.randn` 은 0.0.2a0 에 없어(랜덤
지원은 `acd2a90`, 0.0.3a0 릴리스보다 뒤에 들어왔다 — §0 순서 확인) `cumsum` 기반의 결정적
텐서로 대신했다. RMSNorm(`pow(2)` 경유) → q/k/v/o 프로젝션 → SDPA(causal, gqa) → RMSNorm →
gate/up/down MLP(SiLU), 양쪽 잔차 포함 — **실제 모델 가중치가 아니라는 점을 명시한다.**
워밍업 3, 6~7 회 최솟값, ms (3 회 번갈아 최솟값):

| | old (0.0.2a0) | new (0.0.6a0) | 배수 |
|---|---:|---:|---:|
| `nn.Linear` gate/up 단독, f32 | 2.72875 | 2.72888 | 1.00× |
| `nn.Linear` gate/up 단독, bf16 | 4.81896 | 3.18738 | **1.51×** |
| 합성 블록, f32 | 16.1043 | 14.1563 | **1.14×** |
| 합성 블록, bf16 | 25.2098 | 17.8796 | **1.41×** |

**대조군**: gate/up f32 1.0061×, bf16 1.0076×, 블록 f32 1.0148×, 블록 bf16 1.0137× — 1.5%
이내. 관측 효과(f32 블록 14%, bf16 블록 41%)가 그보다 9~27 배 커서 잡음이 아니다.

**다섯 최적화 전부가 쌓인 결과는 합성 블록에서 f32 14%, bf16 41% 다.** 단일 `nn.Linear`
f32(gate/up)는 사실상 1.00× — 이 형태에서는 GEMM 자체가 지배적이라 디스패치·`pow` 개선이
전체 시간에서 차지하는 몫이 너무 작다는 것과 일치한다. 블록 f32(14%)가 단일 선형(0%)보다
큰 것은 블록에 `pow`(RMSNorm)·SDPA(amax/마스크/복사)·수십 번의 view/transpose(디스패치)가
더해지기 때문이다 — 이 셋이 f32 경로에서 이득을 내는 항목들이고 §10.3 의 GEMM 확장은
빠져 있다는 것과 맞는다. bf16 블록(41%)이 f32 블록(14%)보다 훨씬 큰 것은 §10.3 이 bf16
GEMM 에만 적용되기 때문에 예상된 모양이다.

## 10.6 요약 — 전이 여부와 배수

| 최적화 | 호스트 배수 | 기기 배수(직접 비교 가능한 것) | 판정 |
|---|---:|---:|---|
| #1 바인딩 플랜(`docs/bindings/BIND.md`, 기존 측정) | view 2.91×, transpose 2.77× | view 2.75×, transpose 2.51×, add 1.80× | 전이됨 |
| #2 디스패치 키워드 | view 1.100×, transpose 1.153×, add 1.073× | view 1.19×, transpose 1.19×, add 1.11×, unsqueeze 1.17× | **전이됨, 기기가 더 큼** |
| #3 `pow`→곱셈 | (규모가 달라 직접 대조 안 함, §10.2) | S=128 16.87×, S=512 19.15× (old/new `pow`) | **전이됨** |
| #4 bf16 GEMM 확장 | 모델 수준 2.60~2.68× | 단일 `nn.Linear` 1.49~1.52×, 합성 블록 bf16 1.41× | **전이됨, 모델 수준 숫자보다 작음** |
| #5a `amax` | old/new 로 다시 읽으면 1.19× | SDPA 전체 1.11× | **전이됨, 기기가 호스트보다 작음** |
| #5b/c 융합+복사 | 복사 단독 S=512 5.25×, S=1024 4.84× | 복사 단독 S=512 3.68×, S=1024 3.12×; SDPA 전체(융합+복사 합) 1.11~1.15× | **복사는 전이되지만 호스트에 못 미침** |

**전이되지 않은 항목은 없었다** — 여섯 다 방향이 맞고, 모든 대조군이 0.5~3.2% 인데 관측 효과는
최소 11%(가장 작은 것도 대조군의 3배 이상)다. 다만 **"기기가 항상 호스트보다 크게 이득 본다"는
`docs/bindings/BIND.md` §7 의 주장은 최적화 #4(bf16 GEMM), #5a(`amax`), #5b/c(복사 단독)에서 성립하지
않는다** — 이 셋은 기기 배수가 호스트 배수보다 작다. 성립한 것은 #2(디스패치)뿐이고, #1 도
성립했다(기존 측정). 갈리는 지점은 **파이썬 디스패치 층이 시간을 지배하는가**로 보인다 — #1·#2
는 작은 텐서에 대한 뷰/트랜스포즈/덧셈처럼 커널 자체는 마이크로초 단위이고 파이썬 오버헤드가
대부분을 차지하는 연산이라, 느린 인터프리터가 그 오버헤드의 몫을 키운다는 논리가 그대로
적용된다. #4·#5a·#5b/c 는 GEMM·리덕션·메모리 복사처럼 **커널 자체의 연산/대역폭이 지배하는
연산**이고, 거기서는 인터프리터 속도가 아니라 CPU 의 부동소수점 처리량·메모리 대역폭이 배수를
정하므로 호스트/기기 배수가 같은 방향으로 클 이유가 없다 — 오히려 에뮬레이터가 게스트로서
지불하는 메모리 접근 비용(§0 의 알려진 제약) 때문에 더 작게 나올 수 있다. **"느린 인터프리터가
전이를 키운다"는 인수는 디스패치·바인딩류에만 적용되는 것으로 좁혀 읽어야 하고, 커널
자체가 최적화된 항목에 일반화하면 안 된다 — 이번 측정이 그 일반화가 실제로 깨지는 지점을
직접 보여준다.**
