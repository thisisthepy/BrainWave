...
`docs/BACKWARD7.md` §10 is a list of seven things the eager-recorder round did not establish. Three
...
| **Did the forward move?** | **No.** The three `docs/BACKWARD7.md` §7 digests reproduced from the recipe in the tree. §5 |
...
1. **The guard `docs/BACKWARD7.md` §5 is about never fired.** `note_mutation`'s
...
that `docs/BACKWARD7.md` did not have.

## 3. Randomness: the argument, tested

`docs/BACKWARD7.md` §10 row 5: the eager gating means a dropout mask is *held*, not redrawn, so
`docs/CAPTURE.md` §9-1's failure — a gradient taken at a different draw than the one reported —
...
`docs/ADAPT.md` §14.3 measures it as the dominant error term on a real gpt2 Tent step, by two orders
of magnitude). Running the same program through capture and applying the same comparator disagrees
on almost every element. `docs/CAPTURE.md` §9-1's failure is live in this repository on one path and
absent on the other, and the test now says so with both.

## 4. The tape's lifetime: a bound, a refusal, and a release

`docs/BACKWARD7.md` §10 row 2: the tape is freed by `backward()` and by nothing else, so a forward
...
* **`ps -o rss=` reported `+3.4, +7.2, +11.0` MiB and then `−598.9 MiB`** between two consecutive
  ...
  `docs/BACKWARD7.md` §8 already records this machine corrupting a measurement, which is why the
...
compile inputs. `docs/BACKWARD7.md` §7's recipe is in the tree — sha256 over
...
All three equal `docs/BACKWARD7.md` §9.1's, and through it `docs/BACKWARD6.md` §7's,
`docs/BACKWARD5.md` §6.5.1's and `docs/BACKWARD4.md` §6.2's — a five-round chain, one link of which
...
| 4 | **Anything about `run_backward`, `.grad`, hooks, `create_graph` or `torch.autograd.Function`.** | `docs/BACKWARD7.md` §6 is still the list, and `Tensor.backward()` was explicitly out of scope this round |
